...
ps_RC.rds = "/home/guest/ibiemProject1/ps_RC.rds"
ps_RC_1_12.rds = "/home/guest/ibiemProject1/ps_RC_1_12.rds"
```
...
```{r}
#Subset by Week 
ps_RC_1 <- subset_samples(ps_RC, Week == 1)
ps_RC_12 <- subset_samples(ps_RC, Week == 12)
saveRDS(ps_RC_1,"ps_RC_1.rds")
saveRDS(ps_RC_12,"ps_RC_12.rds")
ps_RC_1_12 <-merge_phyloseq(ps_RC_12,ps_RC_1)
sample_data(ps_RC_1_12)
saveRDS(ps_RC_1_12,"ps_RC_1_12.rds")
```

```{r}
ps_RC
saveRDS(ps_RC, "ps_RC.rds")
sample_data(ps_RC)
```
...
outdir=path.expand("~/scratch/lefse/RC")
...
RepseqToTaxa <- function(ps_RC, ps.ps) {
  ...
  colname_match = match(names(ps_RC), tax.df$repseq)
  cols_to_keep = which(is.na(colname_match))
  colnames_to_keep = names(ps_RC)[cols_to_keep]
  
  # replace repseqs with taxonomies
  names(ps_RC) = tax.df$taxonomy[match(names(ps_RC), tax.df$repseq)]
  # now reset the non-OTU column names
  names(ps_RC)[cols_to_keep] = colnames_to_keep
  return(ps_RC)
}
...
```{r}
#format.flags = GenerateLefseOutput(ps_RC, c("Condition","Week"), lefse.input.file)
grouping_parameter="Condition"
format.flags = GenerateLefseOutput(ps_RC, grouping_parameter, lefse.input.file)
...
```{r}
# poly.ps.rds = "/data/tutorial_data/poly_1pct.rds"
outdir=path.expand("~/scratch/lefse/RC/Week")
# lefse.input.file=file.path(outdir,"data_for_lefse.tsv")
...
ps_RC_1_12 = read_rds(ps_RC_1_12.rds)
print(ps_RC_1_12)
```
...
#' @param df The dataframe generated from phyloseq object.
#' @param ps.ps phyloseq object from which df was derived.
RepseqToTaxa <- function(ps_RC_1_12, ps.ps) {
  tax.df = as.data.frame(tax_table(ps.ps)) %>%
    rownames_to_column("repseq") %>%
    mutate(taxonomy = paste(Kingdom,Phylum,Class,Order,Family,Genus, sep="|")) %>%
    select(repseq, taxonomy)
  ...
  cols_to_keep = which(is.na(colname_match))
  colnames_to_keep = names(ps_RC_1_12)[cols_to_keep]
  
  # replace repseqs with taxonomies
  names(ps_RC_1_12) = tax.df$taxonomy[match(names(ps_RC_1_12), tax.df$repseq)]
  # now reset the non-OTU column names
  names(ps_RC_1_12)[cols_to_keep] = colnames_to_keep
  return(ps_RC_1_12)
}
...
GenerateLefseOutput = function(ps,output_columns,outfile){
  ...
  base_columns = c("SampleID","OTU","Abundance")
  ps_RC_1_12 = psmelt(ps) %>% 
    mutate(SampleID = str_replace(Sample, pattern="\\.", replacement="_")) %>%
    mutate(taxonomy = paste(Kingdom,Phylum,Class,Order,Family,Genus, sep="|")) %>%
    select(one_of(c(base_columns,output_columns))) %>%
    spread(OTU, Abundance)
  
  RepseqToTaxa(ps_RC_1_12, ps) %>%
    write.table(file=outfile, 
                sep="\t", quote = FALSE,
                row.names = FALSE)
...
```{r}
#format.flags = GenerateLefseOutput(ps_RC_1_12, c("Condition","Week"), lefse.input.file)
  grouping_parameter="Week"
format.flags = GenerateLefseOutput(ps_RC_1_12, grouping_parameter, lefse.input.file)
Sys.setenv(FORMAT_FLAGS=format.flags)
```
...
# poly.ps.rds = "/data/tutorial_data/poly_1pct.rds"
outdir=path.expand("~/scratch/lefse/RC/week-Condition")
# lefse.input.file=file.path(outdir,"data_for_lefse.tsv")
...
RepseqToTaxa <- function(ps_RC_1_12, ps.ps) {
  ...
  # need to preserve non-OTU column names (otherwise they get lost)
  colname_match = match(names(ps_RC_1_12), tax.df$repseq)
  cols_to_keep = which(is.na(colname_match))
  colnames_to_keep = names(ps_RC_1_12)[cols_to_keep]
...
```{r}
format.flags = GenerateLefseOutput(ps_RC_1_12, c("Week", "Condition"), lefse.input.file)
#grouping_parameter="Week"
#format.flags = GenerateLefseOutput(ps_RC_1_12, grouping_parameter, lefse.input.file)
Sys.setenv(FORMAT_FLAGS=format.flags)
```
...
```{r}
#Subset by PS and C  
ps_RC_PS <- subset_samples(ps_RC, Condition == "PS")
ps_RC_C <- subset_samples(ps_RC, Condition == "Ctrl")
saveRDS(ps_RC_PS,"ps_RC_PS.rds")
saveRDS(ps_RC_C,"ps_RC_C.rds")
ps_RC_PS_C <-merge_phyloseq(ps_RC_PS,ps_RC_C)
sample_data(ps_RC_PS_C)
saveRDS(ps_RC_PS_C,"ps_RC_PS_C.rds")
```
