...
```{r}
sample_data(poly.ps)
sample_data(poly.ps)["1_2",]
head(rownames(sample_data(poly.ps)))
head(sample_data(poly.ps)$Description)
rownames(sample_data(poly.ps)) == sample_data(poly.ps)$Description

df <- sample_data(poly.ps) %>%
  separate(Description, into = c("Week", "CollectionID"))
data.frame(df)
...
for(i in 1:nrow(sample_data(poly.ps))){
  ...
  collectionID[i] <- strsplit(sample_data(poly.ps)$Description, "_")[[i]][2]
}

for(k in 1:nrow(sample_data(poly.ps))){
  if (collectionID[i] %in% 1:4) site[i] <- "Landfill"
