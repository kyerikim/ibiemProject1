...
```{r}
plot_richness(poly.ps, x="Week", measures=c("Shannon"), color="Condition") + 
  theme_bw() +
  facet_wrap(~Location)
```
...
```{r}
# simpson
plot_richness(poly.ps, x="Week", measures=c("Simpson"), color="Condition") + 
  theme_bw() +
  facet_wrap(~Location)
```
...
top20 <- names(sort(taxa_sums(poly.ps), decreasing=TRUE))[1:20]
ps.top20_rel <- transform_sample_counts(poly.ps, function(OTU) OTU/sum(OTU))
...
plot_bar(ps.top20, x="Week", fill="Family") + 
  geom_bar(stat = "identity", position = "stack", size=0) +
  facet_wrap(~Location + Condition, scales="free_x") 
...
plot_bar(ps_LF, x="Week", fill="Phylum") + 
  geom_bar(stat = "identity", position = "stack", size=0) +
  facet_wrap(~Condition, scales="free_x") 
...
prune.vec = filter_taxa(ps_LF, 
                       function(x) sum(x >= min_count) >= min_sample)
sum(prune.vec)
ps_even_LF = transform_sample_counts(ps_LF, function(x) 1E6 * x/sum(x))
